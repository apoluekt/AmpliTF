...
if __name__ == "__main__" : 
    ...
    with strategy.scope():
        ...
        init_vals = [p.init_value for p in pars]
        print(init_vals)
        kwargs = { p.name : p() for p in float_pars }
        #print("loss ", nll(data_sample, norm_sample, kwargs))
        #loss = strategy.run(nll, args=(dist_values, norm_sample, kwargs,))
        #print(loss)
        #loss = strategy.reduce('SUM', loss, axis=None)
        #print(loss)
        #exit()
        #def ggg(pars):
        #    for i, p in enumerate(float_pars): p.update(init_vals[i])
        #    kwargs = { p.name : p() for p in float_pars }
        #    float_vars = [ i() for i in float_pars ]
        #    with tf.GradientTape() as gt : 
        #        gt.watch( float_vars )
        #        nll_val = nll(data_sample, norm_sample, kwargs)
        ...
        args = (dist_values, norm_sample)
        @atfi.function
        def func(par):
            for i, p in enumerate(float_pars): p.update(par[i])
            kwargs = {p.name : p() for p in float_pars}
            func.n += 1
            dist_args = args + (kwargs,)
            nll_per_replica = strategy.run(nll, args=(dist_args))
            nll_val = strategy.reduce('SUM', nll_per_replica, axis=None)
        ...
        #        return g
        #    ###
        #    return [strategy.reduce('SUM', rep, axis=None) for rep in strategy.run(replica_gt, args=(dist_values,))]
        ...
        #print([(e, type(e))for e in gradient(init_vals)])
        #exit()
    ...
    with open('benchmarks.txt', wmode) as f:
        f.write(f"({argsp.strategy}, gradient: {use_gradient}) --> {results['time']}, {results['time']/results['func_calls']} sec per function call\n")
