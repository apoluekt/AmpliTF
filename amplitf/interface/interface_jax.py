...
def random_uniform(shape, minval, maxval) : 
    global _prng
    _prng, key = rnd.split(_prng)
    return rnd.uniform(key, minval = minval, maxval = maxval, shape = shape)
...
def create_gradient(nll, args, float_pars) : 
    ...
    global _gf
    def grfunc(*pars) : 
        grkwargs = { p.name : pars[i] for i,p in enumerate(float_pars) }
        return nll(*args, grkwargs)
    #_gf = jit(grad(grfunc, range(0, len(float_pars))))
    _gf = grad(grfunc, range(0, len(float_pars)))
    return True
